...
Delta : Get Device Detail all parameter fail [40300]
	#Select Dropdrown Function 
	Select Dropdrown Function    ${ASGARD_COAPAP_IMAGE_DDL_FUNCTION_DELTA}
	
	#Replace Parameters Url IMSI or Token and IP 
	${url}=    Replace Parameters Url Path     ${ASGARD_COAPAPP_URL_STAGING}    ${ASGARD_COAPAPP_URL_DELTA}    ${ASGARD_COAPAPP_FIELD_TOKEN}    ${thingToken}${ASGARD_COAPAPP_VALUE_TST_F3_0_1_003_THINGTOKEN_INVALID}    ${ASGARD_COAPAPP_FIELD_IPADDRESS}    ${ASGARD_COAPAPP_IP_ADDRESS}
	Log To Console    url${url}
...
Delta : Check Detail log [EDR] and Summary Log [CDR]
	...
	${pathUrl}=    Replace Parameters Path    ${ASGARD_COAPAPP_URL_DELTA}    ${ASGARD_COAPAPP_FIELD_TOKEN}    ${thingToken}${ASGARD_COAPAPP_VALUE_TST_F3_0_1_003_THINGTOKEN_INVALID}    ${ASGARD_COAPAPP_FIELD_IPADDRESS}    ${ASGARD_COAPAPP_IP_ADDRESS}
	
	#{"${VALUE_SENSORKEY}": "${random_Sensor}"}  
	${payload}=    Set Variable    null
	
	#api/v1/Delta?ThingToken=8ba1f4bb-0ffc-42f9-b234-ceca95f171c1111&IpAddress=1.2.3.4&Sensor=Temp
	${urlCmdName}=    Set Variable    ${ASGARD_COAPAPP_URL_COAPAPISERVICE_DELTA}?ThingToken=${thingToken}${ASGARD_COAPAPP_VALUE_TST_F3_0_1_003_THINGTOKEN_INVALID}&IpAddress=${ASGARD_COAPAPP_IP_ADDRESS}

	#resultCode_summary[40300],resultDesc_summary[Forbidden],Code_detail[40300],Description_detail[The requested operation could not be found.],applicationName[CoapAPP],pathUrl[/delta/sim/v1/Token/IPAddress],urlCmdName[api/v1/Delta],thingToken,ipAddress,payload[null],namespace[magellanstaging],containerId[coapapp-vXX],identity[null],cmdName[Delta],endPointName[CoapApiService],logLevel[INFO],custom,SensorKey
    Check Log Response    ${VALUE_LOG_CODE_40300}    ${VALUE_DESCRIPTION_FORBIDDEN_ERROR}    ${VALUE_LOG_CODE_40300}    ${VALUE_DESCRIPTION_REQUESTED_OPERATION_COULDNOTBEFOUND_ERROR}    ${VALUE_APPLICATIONNAME_COAPAPP}    ${pathUrl}    ${urlCmdName}    ${thingToken}${ASGARD_COAPAPP_VALUE_TST_F3_0_1_003_THINGTOKEN_INVALID}    ${ASGARD_COAPAPP_IP_ADDRESS}    ${payload}    ${VALUE_LOG_NAMESPACE}    ${VALUE_LOG_CONTAINERID_COAPAPP}    ${VALUE_LOG_SUMMARY_IDENTITY}    ${VALUE_LOG_SUMMARY_CMDNAME_DELTA}    ${DETAIL_ENDPOINTNAME_COAPAPISERVICE}    ${VALUE_LOG_DETAIL_LOGLEVEL}    ${VALUE_LOG_SUMMARY_CUSTOM}    ${getData}[7] 
