...
Config : Get Device Detail all parameter success [40300]
	...
	#Replace Parameters Url IMSI or Token and IP 
	${url}=    Replace Parameters Url Path     ${ASGARD_COAPAPP_URL_STAGING}    ${ASGARD_COAPAPP_URL_CONFIG}    ${ASGARD_COAPAPP_FIELD_TOKEN}    ${thingToken}${ASGARD_COAPAPP_VALUE_TST_F2_0_2_003_THINGTOKEN_INVALID}    ${ASGARD_COAPAPP_FIELD_IPADDRESS}    ${ASGARD_COAPAPP_IP_ADDRESS}
	#Log To Console    url${url}
...
Config : Check Detail log [EDR] and Summary Log [CDR]
	...
	${pathUrlReplace}=    Replace Parameters Path    ${ASGARD_COAPAPP_URL_CONFIG}    ${ASGARD_COAPAPP_FIELD_TOKEN}    ${thingToken}${ASGARD_COAPAPP_VALUE_TST_F2_0_2_003_THINGTOKEN_INVALID}    ${ASGARD_COAPAPP_FIELD_IPADDRESS}    ${ASGARD_COAPAPP_IP_ADDRESS}
	${pathUrl}=    Set Variable    ${pathUrlReplace}
	${payload}=    Set Variable    null
	
	#api/v1/Config?ThingToken=d1b348dc-455b-4578-9d88-ef8f21a3467d&IpAddress=1.2.3.4
	${urlCmdName}=    Set Variable    ${ASGARD_COAPAPP_URL_COAPAPISERVICE_CONFIG}?ThingToken=${thingToken}${ASGARD_COAPAPP_VALUE_TST_F2_0_2_003_THINGTOKEN_INVALID}&IpAddress=${ASGARD_COAPAPP_IP_ADDRESS}

	
	#resultCode_summary[40300],resultDesc_summary[Forbidden],Code_detail[40300],Description_detail[The requested operation could not be found.],applicationName[CoapAPP],pathUrl[/config/sim/v1/token/IP],urlCmdName[api/v1/Config],thingToken,ipAddress,payload[null],namespace[magellanstaging],containerId[coapapp-vXX],identity[null],cmdName[Config],endPointName[CoapApiService],logLevel[INFO],custom,SensorKey
    Check Log Response    ${VALUE_LOG_CODE_40300}    ${VALUE_DESCRIPTION_FORBIDDEN_ERROR}    ${VALUE_LOG_CODE_40300}    ${VALUE_DESCRIPTION_REQUESTED_OPERATION_COULDNOTBEFOUND_ERROR}    ${VALUE_APPLICATIONNAME_COAPAPP}    ${pathUrl}    ${urlCmdName}    ${thingToken}${ASGARD_COAPAPP_VALUE_TST_F2_0_2_003_THINGTOKEN_INVALID}    ${ASGARD_COAPAPP_IP_ADDRESS}    ${payload}    ${VALUE_LOG_NAMESPACE}    ${VALUE_LOG_CONTAINERID_COAPAPP}    ${VALUE_LOG_SUMMARY_IDENTITY}    ${VALUE_LOG_SUMMARY_CMDNAME_CONFIG}    ${DETAIL_ENDPOINTNAME_COAPAPISERVICE}    ${VALUE_LOG_DETAIL_LOGLEVEL}    ${VALUE_LOG_SUMMARY_CUSTOM}    ${getData}[7] 
