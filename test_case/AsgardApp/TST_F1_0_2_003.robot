...
Report : Get Device Detail all parameter fail [40300]
   	#Replace Parameters Url IMSI or Token and IP 
...
Report : Check Detail log [EDR] and Summary Log [CDR]
	...
	${payload}=    Set Variable    {"${getData}[7]":"${randomSensorApp}"}
	
	#resultCode_summary[40300],resultDesc_summary[Forbidden],Code_detail[40300],Description_detail[The requested operation could not be found.],applicationName[CoapAPP],pathUrl[/report/sim/v1/Token/IPAddress],urlCmdName[api/v1/Report],thingToken,ipAddress,payload[null],namespace[magellanstaging],containerId[coapapp-vXX],identity[null],cmdName[Report],endPointName[CoapApiService],logLevel[INFO],custom,SensorKey
    Check Log Response    ${VALUE_LOG_CODE_40300}    ${VALUE_DESCRIPTION_FORBIDDEN_ERROR}    ${VALUE_LOG_CODE_40300}    ${VALUE_DESCRIPTION_REQUESTED_OPERATION_COULDNOTBEFOUND_ERROR}    ${VALUE_APPLICATIONNAME_COAPAPP}    ${pathUrl}    ${ASGARD_COAPAPP_URL_COAPAPISERVICE_REPORT}    ${thingToken}${ASGARD_COAPAPP_VALUE_TST_F1_0_2_003_THINGTOKEN_INVALID}    ${ASGARD_COAPAPP_IP_ADDRESS}    ${payload}    ${VALUE_LOG_NAMESPACE}    ${VALUE_LOG_CONTAINERID_COAPAPP}    ${VALUE_LOG_SUMMARY_IDENTITY}    ${VALUE_LOG_SUMMARY_CMDNAME_REPORT}    ${DETAIL_ENDPOINTNAME_COAPAPISERVICE}    ${VALUE_LOG_DETAIL_LOGLEVEL}    ${VALUE_LOG_SUMMARY_CUSTOM}    ${getData}[7]           
