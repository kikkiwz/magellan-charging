...
Config : Get Device Detail all parameter success [40301]
	...
	#Replace Parameters Url IMSI or Token and IP 
	${url}=    Replace Parameters Url Path     ${ASGARD_COAPAPP_URL_STAGING}    ${ASGARD_COAPAPP_URL_CONFIG}    ${ASGARD_COAPAPP_FIELD_TOKEN}    ${ASGARD_COAPAPI_VALUE_TST_F2_0_2_005_THINGTOKEN}    ${ASGARD_COAPAPP_FIELD_IPADDRESS}    ${ASGARD_COAPAPP_IP_ADDRESS}
	#Log To Console    url${url}
...
Config : Check Detail log [EDR] and Summary Log [CDR]
	...
	${pathUrl}=    Replace Parameters Path    ${ASGARD_COAPAPP_URL_CONFIG}    ${ASGARD_COAPAPP_FIELD_TOKEN}    ${ASGARD_COAPAPI_VALUE_TST_F2_0_2_005_THINGTOKEN}    ${ASGARD_COAPAPP_FIELD_IPADDRESS}    ${ASGARD_COAPAPP_IP_ADDRESS}
	
	${identity}=    Set Variable    {"Imei":"${ASGARD_COAPAPI_VALUE_TST_F2_0_2_005_IMEI}","ThingID":"${ASGARD_COAPAPI_VALUE_TST_F2_0_2_005_THINGID}","Imsi":"${ASGARD_COAPAPI_VALUE_TST_F2_0_2_005_IMSI}"}		
	${custom}=    Evaluate    {'Imei':['${ASGARD_COAPAPI_VALUE_TST_F2_0_2_005_IMEI}'],'url':'coapapis.magellanstaging.svc.cluster.local' + '${ASGARD_COAPAPI_URL_CONFIG}','IpAddress':'${ASGARD_COAPAPP_IP_ADDRESS}','Imsi':['${ASGARD_COAPAPI_VALUE_TST_F2_0_2_005_IMSI}'],'ThingID':['${ASGARD_COAPAPI_VALUE_TST_F2_0_2_005_THINGID}']}
	${body}=    Set Variable    {"ThingToken":"${ASGARD_COAPAPI_VALUE_TST_F2_0_2_005_THINGTOKEN}","IpAddress":"${ASGARD_COAPAPP_IP_ADDRESS}"}
	
	#resultCode_summary[40301],resultDesc_summary[The thingToken requested already exists.],Code_detail[40301],Description_detail[The thingToken requested already exists.],applicationName[Asgard.Coap.APIs],pathUrl[/config/sim/v1/token/IP],urlCmdName[/api/v1/Config],thingToken,ipAddress,body,namespace[magellanstaging],containerId[coapapp-vXX],identity[null],cmdName[Get_Config],endPointName[null],logLevel[INFO],custom,SensorKey
    Check Log Response    ${VALUE_LOG_CODE_40301}    ${VALUE_DESCRIPTION_THETHINGTOKENREQUESTEDALREADYEXISTS_ERROR}    ${VALUE_LOG_CODE_40301}    ${VALUE_DESCRIPTION_THETHINGTOKENREQUESTEDALREADYEXISTS_ERROR}    ${VALUE_APPLICATIONNAME_COAPAPI}    ${pathUrl}    ${ASGARD_COAPAPI_URL_CONFIG}    ${ASGARD_COAPAPI_VALUE_TST_F2_0_2_005_THINGTOKEN}    ${ASGARD_COAPAPP_IP_ADDRESS}    ${body}    ${VALUE_LOG_NAMESPACE}    ${VALUE_LOG_CONTAINERID_COAPAPI}    ${identity}    ${VALUE_LOG_SUMMARY_CMDNAME_GET_CONFIG}    ${EMPTY}    ${VALUE_LOG_DETAIL_LOGLEVEL}    ${custom}    ${getData}[7] 
