...
Config : Get Device Detail all parameter success [40400]
	...
	#Replace Parameters Url IMSI or Token and IP 
	${url}=    Replace Parameters Url Path     ${ASGARD_COAPAPP_URL_STAGING}    ${ASGARD_COAPAPP_URL_CONFIG}    ${ASGARD_COAPAPP_FIELD_TOKEN}    ${ASGARD_COAPAPI_VALUE_TST_F2_1_0_004_THINGTOKEN_INVALID}    ${ASGARD_COAPAPP_FIELD_IPADDRESS}    ${ASGARD_COAPAPP_IP_ADDRESS}
	#Log To Console    url${url}
...
Config : Check Detail log [EDR] and Summary Log [CDR]
	...
	${pathUrl}=    Replace Parameters Path    ${ASGARD_COAPAPP_URL_CONFIG}    ${ASGARD_COAPAPP_FIELD_TOKEN}    ${ASGARD_COAPAPI_VALUE_TST_F2_1_0_004_THINGTOKEN_INVALID}    ${ASGARD_COAPAPP_FIELD_IPADDRESS}    ${ASGARD_COAPAPP_IP_ADDRESS}
	
	${identity}=    Set Variable    {"Imei":null,"ThingID":null,"Imsi":null}		
	${custom}=    Evaluate    {'Imei':${NULL},'url':'coapapis.magellanstaging.svc.cluster.local' + '${ASGARD_COAPAPI_URL_CONFIG}','IpAddress':'${ASGARD_COAPAPP_IP_ADDRESS}','Imsi':${NULL},'ThingID':${NULL}}
