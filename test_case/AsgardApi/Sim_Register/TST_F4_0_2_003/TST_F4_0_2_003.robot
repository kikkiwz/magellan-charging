...
Create Data get IMSI
	...
	#set IMSI
	${setIMSI}=    Set Variable    ${SPACE}
	Set Global Variable    ${IMSI}    ${setIMSI}
	
Register : Device Detail all parameter fail [40400][imsi(ThingIdentifier) is null]
...
Register : Check Detail log [EDR] and Summary Log [CDR]
	...
	${identity}=    Set Variable    {"Imei":null,"ThingID":null,"Imsi":"%20"}		
	${custom}=    Evaluate    {'Imei':${NULL},'url':'coapapis.magellanstaging.svc.cluster.local' + '${ASGARD_COAPAPI_URL_REGISTER}','Imsi':['%20'],'IpAddress':'${ASGARD_COAPAPP_IP_ADDRESS}','ThingID':${NULL}}
	${body}=    Set Variable    {"Imsi":"%20","IpAddress":"${ASGARD_COAPAPP_IP_ADDRESS}"}
    
	#resultCode_summary[40400],resultDesc_summary[Forbidden],Code_detail[40400],Description_detail[The requested operation could not be found.],applicationName[Asgard.Coap.APIs],pathUrl[/register/sim/v1/IMSI/IPAddress],urlCmdName[/api/v1/Sim/Register],imsi,ipAddress,body,namespace[magellanstaging],containerId[coapapp-v26 ],identity[null],cmdName[Register],endPointName[],logLevel[INFO],custom,SensoyKey
    Check Log Response    ${VALUE_LOG_CODE_40400}    ${VALUE_DESCRIPTION_REQUESTED_OPERATION_COULDNOTBEFOUND_ERROR}    ${VALUE_LOG_CODE_40400}    ${VALUE_DESCRIPTION_REQUESTED_OPERATION_COULDNOTBEFOUND_ERROR}    ${VALUE_APPLICATIONNAME_COAPAPI}    ${pathUrl}    ${ASGARD_COAPAPI_URL_REGISTER}    ${ASGARD_COAPAPI_VALUE_TST_F4_0_2_003_DATASEARCH}    ${ASGARD_COAPAPP_IP_ADDRESS}    ${body}    ${VALUE_LOG_NAMESPACE}    ${VALUE_LOG_CONTAINERID_COAPAPI}    ${identity}    ${VALUE_LOG_SUMMARY_CMDNAME_POST_REGISTER}    ${EMPTY}    ${VALUE_LOG_DETAIL_LOGLEVEL}    ${custom}    ${EMPTY}  
