...
Create Data get IMSI
	...
	#set IMSI
	${setIMSI}=    Set Variable    ${ASGARD_COAPAPI_VALUE_TST_F4_0_2_005_IMSI}    
	Set Global Variable    ${IMSI}    ${setIMSI}    
	
Register : Device Detail all parameter fail [40301]
...
Register : Check Detail log [EDR] and Summary Log [CDR]
	...
	${identity}=    Set Variable    {"Imei":"${ASGARD_COAPAPI_VALUE_TST_F4_0_2_005_IMEI}","ThingID":"${ASGARD_COAPAPI_VALUE_TST_F4_0_2_005_THINGID}","Imsi":"${IMSI}"}		
	${custom}=    Evaluate    {'Imei':['${ASGARD_COAPAPI_VALUE_TST_F4_0_2_005_IMEI}'],'url':'coapapis.magellanstaging.svc.cluster.local' + '${ASGARD_COAPAPI_URL_REGISTER}','Imsi':['${IMSI}'],'IpAddress':'${ASGARD_COAPAPP_IP_ADDRESS}','ThingID':['${ASGARD_COAPAPI_VALUE_TST_F4_0_2_005_THINGID}']}
	${body}=    Set Variable    {"Imsi":"${IMSI}","IpAddress":"${ASGARD_COAPAPP_IP_ADDRESS}"}
    
	#resultCode_summary[40301],resultDesc_summary[The ThingIdentifier requested already exists.],Code_detail[40301],Description_detail[The ThingIdentifier requested already exists.],applicationName[Asgard.Coap.APIs],pathUrl[/register/sim/v1/IMSI/IPAddress],urlCmdName[/api/v1/Sim/Register],imsi,ipAddress,body,namespace[magellanstaging],containerId[coapapp-v26 ],identity[null],cmdName[Register],endPointName[],logLevel[INFO],custom,SensorKey
    Check Log Response    ${VALUE_LOG_CODE_40301}    ${VALUE_DESCRIPTION_THETHINGIDENTIFIERREQUESTEDALREADYEXISTS_ERROR}    ${VALUE_LOG_CODE_40301}    ${VALUE_DESCRIPTION_THETHINGIDENTIFIERREQUESTEDALREADYEXISTS_ERROR}    ${VALUE_APPLICATIONNAME_COAPAPI}    ${pathUrl}    ${ASGARD_COAPAPI_URL_REGISTER}    ${IMSI}    ${ASGARD_COAPAPP_IP_ADDRESS}    ${body}    ${VALUE_LOG_NAMESPACE}    ${VALUE_LOG_CONTAINERID_COAPAPI}    ${identity}    ${VALUE_LOG_SUMMARY_CMDNAME_POST_REGISTER}    ${EMPTY}    ${VALUE_LOG_DETAIL_LOGLEVEL}    ${custom}    ${EMPTY}      
