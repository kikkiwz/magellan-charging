...
Create Data get IMSI
	...
	#set IMSI
	${setIMSI}=    Set Variable    ${ASGARD_COAPAPI_VALUE_TST_F4_0_2_007_IMSI}
	Set Global Variable    ${IMSI}    ${setIMSI}
	
Register : Device Detail all parameter fail [40305]
...
Register : Check Detail log [EDR] and Summary Log [CDR]
	...
	${identity}=    Set Variable    {"Imei":"${ASGARD_COAPAPI_VALUE_TST_F4_0_2_007_IMEI}","ThingID":"${ASGARD_COAPAPI_VALUE_TST_F4_0_2_007_THINGID}","Imsi":"${IMSI}"}		
	${custom}=    Evaluate    {'Imei':['${ASGARD_COAPAPI_VALUE_TST_F4_0_2_007_IMEI}'],'url':'coapapis.magellanstaging.svc.cluster.local' + '${ASGARD_COAPAPI_URL_REGISTER}','Imsi':['${IMSI}'],'IpAddress':'${ASGARD_COAPAPP_IP_ADDRESS}','ThingID':['${ASGARD_COAPAPI_VALUE_TST_F4_0_2_007_THINGID}']}
	${body}=    Set Variable    {"Imsi":"${IMSI}","IpAddress":"${ASGARD_COAPAPP_IP_ADDRESS}"}
	
	#resultCode_summary[40305],resultDesc_summary[Forbidden],Code_detail[40305],Description_detail[The requested operation could not be found.],applicationName[Asgard.Coap.APIs],pathUrl[/register/sim/v1/IMSI/IPAddress],urlCmdName[/api/v1/Sim/Register],imsi,ipAddress,body,namespace[magellanstaging],containerId[coapapp-v26 ],identity[null],cmdName[Register],endPointName[],logLevel[INFO],custom,sensorKey
