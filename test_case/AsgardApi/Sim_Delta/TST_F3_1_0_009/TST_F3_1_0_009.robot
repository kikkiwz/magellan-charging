...
Delta : Get Device Detail all parameter fail [40305]
	#Select Dropdrown Function 
	Select Dropdrown Function    ${ASGARD_COAPAP_IMAGE_DDL_FUNCTION_DELTA}
	
	#Replace Parameters Url IMSI or Token and IP 
	${url}=    Replace Parameters Url Path     ${ASGARD_COAPAPP_URL_STAGING}    ${ASGARD_COAPAPP_URL_DELTA}    ${ASGARD_COAPAPP_FIELD_TOKEN}    ${ASGARD_COAPAPI_VALUE_TST_F3_1_0_009_THINGTOKEN}    ${ASGARD_COAPAPP_FIELD_IPADDRESS}    ${ASGARD_COAPAPP_IP_ADDRESS}
	#Log To Console    url${url}
...
Delta : Check Detail log [EDR] and Summary Log [CDR]
	...
	${pathUrl}=    Replace Parameters Path    ${ASGARD_COAPAPP_URL_DELTA}    ${ASGARD_COAPAPP_FIELD_TOKEN}    ${thingToken}    ${ASGARD_COAPAPP_FIELD_IPADDRESS}    ${ASGARD_COAPAPP_IP_ADDRESS}
	
	${identity}=    Set Variable    {"Imei":"${ASGARD_COAPAPI_VALUE_TST_F3_1_0_009_IMEI}","ThingID":"${ASGARD_COAPAPI_VALUE_TST_F3_1_0_009_THINGID}","Imsi":"${ASGARD_COAPAPI_VALUE_TST_F3_1_0_009_IMSI}"}		
	${custom}=    Evaluate    {'Imei':['${ASGARD_COAPAPI_VALUE_TST_F3_1_0_009_IMEI}'],'url':'coapapis.magellanstaging.svc.cluster.local' + '${ASGARD_COAPAPI_URL_DELTA}','IpAddress':'${ASGARD_COAPAPP_IP_ADDRESS}','Imsi':['${ASGARD_COAPAPI_VALUE_TST_F3_1_0_009_IMSI}'],'ThingID':['${ASGARD_COAPAPI_VALUE_TST_F3_1_0_009_THINGID}']}
	${body}=    Set Variable    {"ThingToken":"${ASGARD_COAPAPI_VALUE_TST_F3_1_0_009_THINGTOKEN}","IpAddress":"${ASGARD_COAPAPP_IP_ADDRESS}"}

	#resultCode_summary[40305],resultDesc_summary[The operation has already expired.],Code_detail[40305],Description_detail[The operation has already expired.],applicationName[Asgard.Coap.APIs],pathUrl[/config/sim/v1/token/IP],urlCmdName[/api/v1/Config],thingToken,ipAddress,body,namespace[magellanstaging],containerId[coapapp-vXX],identity[null],cmdName[Get_Delta],endPointName[null],logLevel[INFO],custom,SensorKey
    Check Log Response    ${VALUE_LOG_CODE_40305}    ${VALUE_DESCRIPTION_THEOPERATIONHASALREADYEXPIRED_ERROR}    ${VALUE_LOG_CODE_40305}    ${VALUE_DESCRIPTION_THEOPERATIONHASALREADYEXPIRED_ERROR}    ${VALUE_APPLICATIONNAME_COAPAPI}    ${pathUrl}    ${ASGARD_COAPAPI_URL_DELTA}    ${ASGARD_COAPAPI_VALUE_TST_F3_1_0_009_THINGTOKEN}    ${ASGARD_COAPAPP_IP_ADDRESS}    ${body}    ${VALUE_LOG_NAMESPACE}    ${VALUE_LOG_CONTAINERID_COAPAPI}    ${identity}    ${VALUE_LOG_SUMMARY_CMDNAME_GET_DELTA}    ${EMPTY}    ${VALUE_LOG_DETAIL_LOGLEVEL}    ${custom}    ${getData}[7]       
