...
Delta : Get Device Detail all parameter fail [40400]
	...
	#Replace Parameters Url IMSI or Token and IP 
	${url}=    Replace Parameters Url Path     ${ASGARD_COAPAPP_URL_STAGING}    ${ASGARD_COAPAPP_URL_DELTA}    ${ASGARD_COAPAPP_FIELD_TOKEN}    ${ASGARD_COAPAPI_VALUE_TST_F3_1_0_004_THINGTOKEN_INVALID}    ${ASGARD_COAPAPP_FIELD_IPADDRESS}    ${ASGARD_COAPAPP_IP_ADDRESS}
	#Log To Console    url${url}
...
Delta : Check Detail log [EDR] and Summary Log [CDR]
	...
	${pathUrl}=    Replace Parameters Path    ${ASGARD_COAPAPP_URL_DELTA}    ${ASGARD_COAPAPP_FIELD_TOKEN}    ${ASGARD_COAPAPI_VALUE_TST_F3_1_0_004_THINGTOKEN_INVALID}    ${ASGARD_COAPAPP_FIELD_IPADDRESS}    ${ASGARD_COAPAPP_IP_ADDRESS}
	
	${identity}=    Set Variable    {"Imei":null,"ThingID":null,"Imsi":null}		
	${custom}=    Evaluate    {'Imei':${NULL},'url':'coapapis.magellanstaging.svc.cluster.local' + '${ASGARD_COAPAPI_URL_DELTA}','IpAddress':'${ASGARD_COAPAPP_IP_ADDRESS}','Imsi':${NULL},'ThingID':${NULL}}
