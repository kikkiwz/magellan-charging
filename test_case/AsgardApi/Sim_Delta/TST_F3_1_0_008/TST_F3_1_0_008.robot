...
Delta : Get Device Detail all parameter fail [40400]
	...
	#Url
	Input Url    ${ASGARD_COAPAP_IMAGE_TXT_URL_DELTA}    ${url}?${ASGARD_COAPAPI_VALUE_TST_F3_1_0_008_SENSOR_INVALID}    ${ASGARD_COAPAP_IMAGE_TXT_URL_NULL}
	#Click Send button
	Click    ${ASGARD_COAPAP_IMAGE_BTN_SEND}
	#Check Success popup
	Check Matched    ${ASGARD_COAPAP_IMAGE_POPUP_SUCCESS}
...
Delta : Check Detail log [EDR] and Summary Log [CDR]
	...
	${pathUrl}=    Set Variable    ${pathUrlReplace}?${ASGARD_COAPAPI_VALUE_TST_F3_1_0_008_SENSOR_INVALID}
	
	${identity}=    Set Variable    {"Imei":"${IMSI}","ThingID":"${getData}[3]","Imsi":"${IMSI}"}		
	${custom}=    Evaluate    {'Imei':['${IMSI}'],'url':'coapapis.magellanstaging.svc.cluster.local' + '${ASGARD_COAPAPI_URL_DELTA}','IpAddress':'${ASGARD_COAPAPP_IP_ADDRESS}','Imsi':['${IMSI}'],'ThingID':['${getData}[3]']}
	${body}=    Set Variable    {"ThingToken":"${thingToken}","IpAddress":"${ASGARD_COAPAPP_IP_ADDRESS}","Sensor":"${ASGARD_COAPAPI_VALUE_TST_F3_1_0_008_SENSOR_INVALID}"}
	
	#resultCode_summary[40400],resultDesc_summary[The requested operation could not be found.],Code_detail[40400],Description_detail[The requested operation could not be found.],applicationName[Asgard.Coap.APIs],pathUrl[/config/sim/v1/token/IP],urlCmdName[/api/v1/Config],thingToken,ipAddress,body,namespace[magellanstaging],containerId[coapapp-vXX],identity[null],cmdName[Get_Delta],endPointName[null],logLevel[INFO],custom,SensorKey
    Check Log Response    ${VALUE_LOG_CODE_40400}    ${VALUE_DESCRIPTION_REQUESTED_OPERATION_COULDNOTBEFOUND_ERROR}    ${VALUE_LOG_CODE_40400}    ${VALUE_DESCRIPTION_REQUESTED_OPERATION_COULDNOTBEFOUND_ERROR}    ${VALUE_APPLICATIONNAME_COAPAPI}    ${pathUrl}    ${ASGARD_COAPAPI_URL_DELTA}    ${thingToken}${ASGARD_COAPAPI_VALUE_TST_F3_1_0_003_THINGTOKEN_INVALID}    ${ASGARD_COAPAPP_IP_ADDRESS}    ${body}    ${VALUE_LOG_NAMESPACE}    ${VALUE_LOG_CONTAINERID_COAPAPI}    ${identity}    ${VALUE_LOG_SUMMARY_CMDNAME_GET_DELTA}    ${ASGARD_COAPAPI_VALUE_TST_F3_1_0_008_SENSOR_INVALID}    ${VALUE_LOG_DETAIL_LOGLEVEL}    ${custom}    ${getData}[7] 
