...
Report : Get Device Detail all parameter success [40305]
	#Replace Parameters Url IMSI or Token and IP 
	${url}=    Replace Parameters Url Path     ${ASGARD_COAPAPP_URL_STAGING}    ${ASGARD_COAPAPP_URL_REPORT}    ${ASGARD_COAPAPP_FIELD_TOKEN}    ${ASGARD_COAPAPI_VALUE_TST_F1_0_2_007_THINGTOKEN}    ${ASGARD_COAPAPP_FIELD_IPADDRESS}    ${ASGARD_COAPAPP_IP_ADDRESS}
	#Log To Console    url${url}
...
Report : Check Detail log [EDR] and Summary Log [CDR]
	...
	${identity}=    Set Variable    {"Imei":"${ASGARD_COAPAPI_VALUE_TST_F1_0_2_007_IMEI}","ThingID":"${ASGARD_COAPAPI_VALUE_TST_F1_0_2_007_THINGID}","Imsi":"${ASGARD_COAPAPI_VALUE_TST_F1_0_2_007_IMSI}"}		
	${custom}=    Evaluate    {'Imei':['${ASGARD_COAPAPI_VALUE_TST_F1_0_2_007_IMEI}'],'url':'coapapis.magellanstaging.svc.cluster.local' + '${ASGARD_COAPAPI_URL_REPORT}','IpAddress':'${ASGARD_COAPAPP_IP_ADDRESS}','Imsi':['${ASGARD_COAPAPI_VALUE_TST_F1_0_2_007_IMSI}'],'ThingID':['${ASGARD_COAPAPI_VALUE_TST_F1_0_2_007_THINGID}']}
	${body}=    Set Variable    {"ThingToken":"${ASGARD_COAPAPI_VALUE_TST_F1_0_2_007_THINGTOKEN}","IpAddress":"${ASGARD_COAPAPP_IP_ADDRESS}","Payloads":${valueKey},"UnixTimestampMs":[tid]}
	
	#resultCode_summary[40305],resultDesc_summary[The operation has already expired.],Code_detail[40305],Description_detail[The operation has already expired.],applicationName[Asgard.Coap.APIs],pathUrl[/register/sim/v1/IMSI/IPAddress],urlCmdName[/api/v1/Register],imsi,ipAddress,body,namespace[magellanstaging],containerId[coapapp-vXX],identity[null],cmdName[Post_Report],endPointName[],logLevel[INFO],custom,SensorKey
    Check Log Response    ${VALUE_LOG_CODE_40305}    ${VALUE_DESCRIPTION_THEOPERATIONHASALREADYEXPIRED_ERROR}    ${VALUE_LOG_CODE_40305}    ${VALUE_DESCRIPTION_THEOPERATIONHASALREADYEXPIRED_ERROR}    ${VALUE_APPLICATIONNAME_COAPAPI}    ${pathUrl}    ${ASGARD_COAPAPI_URL_REPORT}    ${ASGARD_COAPAPI_VALUE_TST_F1_0_2_007_THINGTOKEN}    ${ASGARD_COAPAPP_IP_ADDRESS}    ${body}    ${VALUE_LOG_NAMESPACE}    ${VALUE_LOG_CONTAINERID_COAPAPI}    ${identity}    ${VALUE_LOG_SUMMARY_CMDNAME_POST_REPORT}    ${EMPTY}    ${VALUE_LOG_DETAIL_LOGLEVEL}    ${custom}    ${getData}[7]  
    #Log To Console    logthingToken${thingToken}
Remove Data
