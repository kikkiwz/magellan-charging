...
Report : Get Device Detail all parameter success [40400]
	#Replace Parameters Url IMSI or Token and IP 
	${url}=    Replace Parameters Url Path     ${ASGARD_COAPAPP_URL_STAGING}    ${ASGARD_COAPAPP_URL_REPORT}    ${ASGARD_COAPAPP_FIELD_TOKEN}    ${ASGARD_COAPAPI_VALUE_TST_F1_0_2_003_THINGTOKEN}    ${ASGARD_COAPAPP_FIELD_IPADDRESS}    ${ASGARD_COAPAPP_IP_ADDRESS}
	#Log To Console    url${url}
...
Report : Check Detail log [EDR] and Summary Log [CDR]
	...
	${pathUrlReplace}=    Replace Parameters Path    ${ASGARD_COAPAPP_URL_REPORT}    ${ASGARD_COAPAPP_FIELD_TOKEN}    ${ASGARD_COAPAPI_VALUE_TST_F1_0_2_003_THINGTOKEN}    ${ASGARD_COAPAPP_FIELD_IPADDRESS}    ${ASGARD_COAPAPP_IP_ADDRESS}
	...
	${custom}=    Evaluate    {'Imei':${NULL},'url':'coapapis.magellanstaging.svc.cluster.local' + '${ASGARD_COAPAPI_URL_REPORT}','IpAddress':'${ASGARD_COAPAPP_IP_ADDRESS}','Imsi':${NULL},'ThingID':${NULL}}
	${body}=    Set Variable    {"ThingToken":"${ASGARD_COAPAPI_VALUE_TST_F1_0_2_003_THINGTOKEN}","IpAddress":"${ASGARD_COAPAPP_IP_ADDRESS}","Payloads":{"${getData}[7]":"${setValueKey}"},"UnixTimestampMs":[tid]}
	
	#resultCode_summary[40400],resultDesc_summary[The requested operation was successfully.],Code_detail[40400],Description_detail[The requested operation was successfully.],applicationName[Asgard.Coap.APIs],pathUrl[/register/sim/v1/IMSI/IPAddress],urlCmdName[/api/v1/Register],imsi,ipAddress,body,namespace[magellanstaging],containerId[coapapp-vXX],identity[null],cmdName[Post_Report],endPointName[],logLevel[INFO],custom,SensorKey
    Check Log Response    ${VALUE_LOG_CODE_40400}    ${VALUE_DESCRIPTION_REQUESTED_OPERATION_COULDNOTBEFOUND_ERROR}    ${VALUE_LOG_CODE_40400}    ${VALUE_DESCRIPTION_REQUESTED_OPERATION_COULDNOTBEFOUND_ERROR}    ${VALUE_APPLICATIONNAME_COAPAPI}    ${pathUrl}    ${ASGARD_COAPAPI_URL_REPORT}    ${ASGARD_COAPAPI_VALUE_TST_F1_0_2_003_THINGTOKEN}    ${ASGARD_COAPAPP_IP_ADDRESS}    ${body}    ${VALUE_LOG_NAMESPACE}    ${VALUE_LOG_CONTAINERID_COAPAPI}    ${identity}    ${VALUE_LOG_SUMMARY_CMDNAME_POST_REPORT}    ${EMPTY}    ${VALUE_LOG_DETAIL_LOGLEVEL}    ${custom}    ${getData}[7] 
    #Log To Console    logthingToken${thingToken}
