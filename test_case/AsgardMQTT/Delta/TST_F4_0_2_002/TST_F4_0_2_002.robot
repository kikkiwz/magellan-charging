...
User Delta Per Sensor parameter fail [40300]	
	#Click Disconnect
	...
	Input Text    ${ASGARD_MQTT_IMAGE_TXT_PUBLISH_TOPIC_AFTERCLEAR}    ${urlPulishReport}/${getData}[7]
	
    #Replace Parameters Url ThingToken
	${urlSubscriptionDeltaReplace}=    Replace Parameters    ${ASGARD_MQTT_URL_DELTA_TOPIC_SUBSCRIPTION}    ${ASGARD_MQTT_FIELD_TOKEN}    ${ThingToken}
	${urlSubscriptionDelta}=    Replace Parameters    ${urlSubscriptionDeltaReplace}    ${ASGARD_MQTT_FIELD_SENSORNAME}    ${ASGARD_MQTT_VALUE_TST_F4_0_2_002_SENSORNAME}
	#Log To Console    urlSubscriptionDelta${urlSubscriptionDelta}	
	...
	Type With Modifiers    a    KEY_CTRL
	Type With Modifiers    x    KEY_CTRL
	Input Text    ${ASGARD_MQTT_IMAGE_TXT_SUBSCRIPTION_TOPIC_NULL}    ${urlSubscriptionDelta}
	Click    ${ASGARD_MQTT_IMAGE_BTN_SUBSCRIBE}
...
Delta : Check Detail log [EDR] and Summary Log [CDR]
	...
	${pathUrl}=    Replace String    ${pathUrlReplace}/${getData}[7]    /    .
	
	#delta/sim/v1/ThingToken/SensorName
	${urlCmdNameReplaceThingToken}=    Replace String    ${ASGARD_MQTT_URL_DELTA_TOPIC_SUBSCRIPTION}    ${ASGARD_MQTT_FIELD_TOKEN}    ${thingToken}
	${urlCmdNameReplaceSensorName}=    Replace String    ${urlCmdNameReplaceThingToken}    ${ASGARD_MQTT_FIELD_SENSORNAME}    ${getData}[7]
	...
	#resultCode_summary[40300],resultDesc_summary[The requested operation was successfully.],Code_detail[40300],Description_detail[Register is Success],applicationName[Asgard.Mqtt.V1.APP],pathUrl[/register/sim/v1/IMSI/IPAddress],urlCmdName[/api/v1/Sim/Register],ThingToken,ipAddress,body,namespace[magellanstaging],containerId[coapapp-vXX],identity[null],cmdName[report],endPointName[RabbitMQ],logLevel[INFO],valueSearchText,custom,SensorKey
    MQTT Check Log Response    ${VALUE_LOG_CODE_40300}    ${VALUE_DESCRIPTION_FORBIDDEN_ERROR}    ${VALUE_LOG_CODE_40300}    ${VALUE_DESCRIPTION_REQUESTED_OPERATION_COULDNOTBEFOUND_ERROR}    ${VALUE_APPLICATIONNAME_MQTT}    ${pathUrl}    ${urlCmdName}    ${ThingToken}    ${ASGARD_MQTT_IP_ADDRESS}    ${body}    ${VALUE_LOG_NAMESPACE}    ${VALUE_LOG_CONTAINERID_MQTT}    ${VALUE_LOG_SUMMARY_IDENTITY}    ${VALUE_LOG_SUMMARY_CMDNAME_REPORT_MQTT}    ${DETAIL_ENDPOINTNAME_RABBITMQ}    ${VALUE_LOG_DETAIL_LOGLEVEL}    ${VALUE_SEARCH_REPORT_MQTT}    ${VALUE_LOG_SUMMARY_CUSTOM}    ${getData}[7]                
    #Log To Console    logthingToken${thingToken}
